...
| B3 workers 0/2/4/8/12 | 全 137,681 | ~13.6 | 同 | **0%**（workers=0 都喂得饱 → 数据管线非瓶颈）⚠️ **该结论已作废，见下表后 2026-09-12 更正** |
...
> ⚠️ **2026-09-12 更正：该结论作废，不可引用。** 根因：perf_bench 的 BASE_ARGS 末尾有 `--num-workers 8`，argparse 取最后一个 → B3_dl_w0/w2/w4/w12 的 workers 全被静默覆盖为 8，四个变体从未真正测过 0/2/4/12。历史 CSV 中 B0/B1/B2/B3×5/B5 共 8 行 tokens/s 全为 137681.0 即由此而来。已修复（删除重复 flag + 新增 duplicate_flags 检查），**待重跑后重新判定 workers 影响**。
>
> 对应上面 6.1 结果总表第 5 行 `B3 workers 0/2/4/8/12`：该行的 `全 137,681`、`**0%**`、「workers=0 都喂得饱 → 数据管线非瓶颈」均不可引用。为不破坏表格结构，更正标注紧随表格之后而非插在表格行之间。
...
2. CPU 侧全部零收益（logging/数据管线/workers/fused/bf16/batch）：GPU 图效率是唯一瓶颈——P0/P1 修复保留（正确性 + 未来更大数据扩展），但对当前吞吐无贡献
   > ⚠️ **2026-09-12 更正：本项中「数据管线 / workers 零收益」的部分作废，不可引用。** B3 的 w0/w2/w4/w12 四个变体被 `--num-workers 8` 静默覆盖（详见 6.1 表后更正），故「workers=0 都喂得饱 → 数据管线非瓶颈」不成立，**待重跑后重新判定**；本项其余部分（logging/fused/bf16/batch）不在本次更正范围。
