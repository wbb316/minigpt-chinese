...
> 以下两行是**追加模板**：下次训练（v2，配置见 `experiment_config_v2.yaml`）完成后，
> 复制模板行、替换 `<…>` 占位符、追加到上方表格末尾，并把本注释删掉。

| 日期 | commit 版本 | 模型配置 | 数据版本 | token 数量 | 训练时间 | best ckpt val loss | 备注 |
|---|---|---|---|---|---|---|---|
| `<YYYY-MM-DD>` | `<git short hash>` | `<L/H/d/bs/vocab/tie>` | `<数据版本>` | `<token 数>` | `<起止/时长>` | `<val loss>` | `<亮点/坑/对比>` |
