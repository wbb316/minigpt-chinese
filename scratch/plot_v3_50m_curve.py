# -*- coding: utf-8 -*-
"""画 v3 50M 训练曲线（单轮 30,455 步，rope+GPT-2 init）：EMA train loss + val loss
→ log/50M参数_v3_alpha+998Mtokens/training_curve_v3.png
仿 v2 50M 的 training_curve_v2.png 风格。
⚠️ step_history CSV 混有 35M(E1/E2)/50M E1/rope 短训行 —— 按 wall_time >= 2026-09-08 16:55 过滤出 v3。
"""
import os
...
OUT = os.path.join(LOGD, 'training_curve_v3_alpha.png')

T0 = '2026-09-08 16:55'   # v3 run 起点（此前的 rope 短训 11:05-11:14，互不重叠）
...
ax.set_title('v3 50M training curve — webnovel_v2 shard0, RoPE + GPT-2 init (12L/576d/9H, 51.4M)')
ax.legend(loc='upper right', fontsize=9)
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(OUT, dpi=130)
print('saved:', OUT)
print(f'v3 steps: {len(train_rows)}, val points: {len(val)}, '
      f'best val {b["val"]:.4f} @ step {int(b["step"])}')
