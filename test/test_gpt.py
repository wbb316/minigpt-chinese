...
def test_gpt_out_shape():
    ...
    torch.manual_seed(0)
    # 非 FFN 测试：显式固定 ff_type，避免 DEFAULT_FF_TYPE 变更引起行为漂移
    gpt=GPT(vocab_size=100,n_layer=2,n_head=4,n_embd=32,block_size=64,ff_type='relu')
...
def test_gpt_different_positions_differ():
    ...
    torch.manual_seed(0)
    # 测位置编码而非 FFN：显式固定 ff_type，避免默认值变更引起行为漂移
    gpt = GPT(vocab_size=50, n_layer=2, n_head=2, n_embd=32, block_size=32,
              position_encoding='sinusoidal', ff_type='relu')
    gpt.eval()
